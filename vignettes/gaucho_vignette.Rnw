...
This vignette serves as an introduction to the R package gaucho.  It covers the basic use cases and usage of the package, explaining the input and output and contains several worked examples, including how to massively speed up computation using parallelism.  If you use this package, please cite it\cite{gauchoR} and/or a recent paper where it was used\cite{melchor}.  A BibTeX entry for LaTeX users is:
...
\noindent \url{https://github.com/MurisonWardell/gaucho}

\paragraph{Dependencies:} Please note that gaucho depends on the following packages which are either available from CRAN or part of base R, but these should be automatically downloaded when Gaucho is installed using biocLite():
...
\begin{enumerate}
   ...
   \item text file: {\color{icrred}k}clones.diversity.indices.solution.{\color{icrred}i}of{\color{icrred}n}.txt: four different diversity indices calculated for each sample.  The Shannon, Simpson, Gini-Simpson and Berger-Parker indices
\end{enumerate}
...
Four diversity indices are calculated for each sample.  These are quantitative measures of how many clones are present and their relative proportions.  Wikipedia has a good primer on the subject: \url{http://en.wikipedia.org/wiki/Diversity_index}
...
\begin{enumerate}
   ...
   \item The input data is contaminated, so we can test the \texttt{contamination} argument of \texttt{gaucho()} 
...
\subsubsection{gaucho and parallelism}
Taking advantage of the parallelism built into gaucho is essential for speed.  This has mainly been achieved by utilising code already implemented in the GA package.  All that is required of the user is ensuring that the correct packages are installed (if they are not, warnings will be issued and you will be informed which packages to install) and to add the \texttt{parralel}  argument to any \texttt{gaucho()} commands.  Parallel support is platform independant and should work on Windows, Linux and OSX.

<<parallel, eval=FALSE>>=
## Load libraries; include the rbenchmark() library to test speed
library(gaucho)
library(rbenchmark)
...
## Benchmark gaucho using default settings and increasing numbers of cores (run on a server with 2 Intel Xeon X5650 CPUs)
## 500 iterations isn't enough to find a good solution, we just want to see how fast it's running
within(benchmark(Linear=gaucho(gaucho_synth_data,number_of_clones=6,iterations=500,parallel=FALSE),
                 Par2=gaucho(gaucho_synth_data,number_of_clones=6,iterations=500,parallel=2),
                 Par4=gaucho(gaucho_synth_data,number_of_clones=6,iterations=500,parallel=4),
                 Par8=gaucho(gaucho_synth_data,number_of_clones=6,iterations=500,parallel=8),
                 Par16=gaucho(gaucho_synth_data,number_of_clones=6,iterations=500,parallel=16),
                 Par20=gaucho(gaucho_synth_data,number_of_clones=6,iterations=500,parallel=20),
                 replications=5,
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{## Output from the benchmark above:}

    test replications        elapsed      average
1 Linear            5       1398.390      279.6780
2   Par2            5       1109.591      221.9182
3   Par4            5        731.923      146.3846
4   Par8            5        562.404      112.4808
5  Par16            5        504.422      100.8844
6  Par20            5        524.223      104.8446
...
% x=c(1,2,4,8,16,20)
% y=c(279.6780,221.9182,146.3846,112.4808,100.8844,104.8446)
% plot(x,y,type="l",ylab="Time (seconds)",xlab="Number of cores")
% points(x,y,pch=15)

\begin{figure}[H]
   \centering
       \includegraphics[width=\textwidth]{parallel_cores}
       \caption{Time decreases as number of cores increases, up to a point.}
\end{figure}

Parallelising a problem is not as simple as throwing more cores at it.  Time will be wasted by breaking up the problem and communicating with cores if too many cores are used.  This depends on the complexity of the problem; in this example, the synthetic data used contains four samples and 90 SNVs; a larger and more complex problem (i.e. more samples and/or more SNVs) may benefit from more cores.  It seems clear that 2-8 cores will lead to significant speed increases, which is particularly relevant to users of desktop hardware.  If in doubt, set \texttt{parralel=TRUE} which will use all the available cores on your machine.
